...
var JJournal = {
    ...
    validerJournal: function(id,retour) {
        var bValid = true;
        $('.ui-corner-all').removeClass("ui-state-error");
        bValid = bValid && JJournal.checkinput($("#codejournal"));
        bValid = bValid && JJournal.checkcombo($("#libelle"));

        if (bValid) {
            $.get(JJournal.urlajax, {
                action: 'validerJournal',
                id: id,
                retour: retour,
            ...
            buttons: {
                OUI: function() {
                    $.get(JJournal.urlajax, {
                        ...
                        switch (data) {
                            case '0':
                                JJournal.updateTips("<div style=height:30px;>Erreur de connexion</div>");
                                break;
                            case '1':
    ...
    checkinput: function(o) {
        if (o.val().length == 0) {
            ...
            JJournal.updateTips("<div style=height:30px;>Veuillez remplir ce champ. SVP!</div>");
            return false;
        } else {
            return true;
        }
    ...
    checkcombo: function(o) {
        if (o.val() == 0) {
            o.addClass("ui-state-error");
            JJournal.updateTips("<div style=height:30px;>Veuillez effectuer un choix. SVP!</div>");
            return false;
        } else {
            return true;
        }
