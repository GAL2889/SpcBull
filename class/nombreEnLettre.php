    ...
    */
    public static function ConvNumberLetter($Nombre, $Devise = 0, $Langue = 0) {
        ...
        if (($dblEnt > 1) && ($Devise != 0)) $strDev = $strDev . "s" ;
            ...
            $NumberLetter = nombreEnLettre::ConvNumEnt(floatval($dblEnt), $Langue) . $strDev . " " . nombreEnLettre::ConvNumDizaine($byDec, $Langue) . $strCentimes ;
            return $NumberLetter;
    }

    private static function ConvNumEnt($Nombre, $Langue) {
       $byNum=$iTmp=$dblReste='' ;
       $StrTmp = '';
       $NumEnt='' ;
        $iTmp = $Nombre - (intval($Nombre / 1000) * 1000) ;
        $NumEnt = nombreEnLettre::ConvNumCent(intval($iTmp), $Langue) ;
        $dblReste = intval($Nombre / 1000) ;
        $iTmp = $dblReste - (intval($dblReste / 1000) * 1000) ;
        $StrTmp = nombreEnLettre::ConvNumCent(intval($iTmp), $Langue) ;
        switch($iTmp) {
            case 0 :
                            break;
            case 1 :
    ...
    private static function ConvNumDizaine($Nombre, $Langue) {
        $TabUnit=$TabDiz='';
        $byUnit=$byDiz='' ;
        $strLiaison = '' ;
    ...
    private static function ConvNumCent($Nombre, $Langue) {
        $TabUnit='' ;
        $byCent=$byReste='' ;
        $strReste = '' ;
        $NumCent='';
        ...
        $strReste = nombreEnLettre::ConvNumDizaine($byReste, $Langue);
        switch($byCent) {
            case 0 :
                $NumCent = $strReste ;
                            break;
