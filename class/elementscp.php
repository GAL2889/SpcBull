<?php

/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */

/**
 * Description of bureaux
 *
 * @author FIDELE
 */
class elementscp {

    //put your code here
    var $id;
    var $codeunique;
    var $idtypeelement;
    var $designation;
    var $idcreateur;
    
    public function __construct($codeunique){
        $rekete = "select * from elementscp where codeunique = '" . $codeunique . "'";
        $result = Functions::commit_sql($rekete,"");
        
        if ($result){
            
            $list = $result-> fetch();
            $this->id = $list["id"];
            $this->codeunique = $list["codeunique"];
            $this->idtypeelement = $list["idtypeelement"];
            $this->designation = $list["designation"];
            $this->idcreateur = $list["idcreateur"];
                       
        }
        
    }
   
    public function getId() {
        return $this->id;
    }

    public function setId($id) {
        $this->id = $id;
    }

    public function getCodeunique() {
        return $this->codeunique;
    }

    public function setCodeunique($codeunique) {
        $this->codeunique = $codeunique;
    }
        
    public function getIdTypeelement(){
        return $this->idtypeelement;
    }
    
    public function setIdtypeelement($idtypeelement){
        $this->idtypeelement = $idtypeelement;
    }
    
    public function getDesignation(){
        return $this->Designation();
    }
    
    public function setDesignation($designation){
        $this->designation = $designation;
    }
    
    public function getIdcreateur(){
        return $this->Idcreateur();
    }
    
    public function setIdcreateur($idcreateur){
        $this->idcreateur = $idcreateur;
    }
    
    public static function libTypeElement($idtypelement){
        $rekete = "SELECT * FROM typeelementscp WHERE id ='".$idtypelement."'";
        $result = Functions::commit_sql($rekete, "");
        $ret = "";   //     echo $rekete;
        if($result){
            $ls = $result->fetch();
            $ret = $ls['libelle'];
        }
        return $ret;
    }

        public function verifDoublon(){
      $rekete = "select * from elementscp where designation = '" . $this->designation ."' AND idtypeelement = '" . $this->idtypeelement ."'";
      $result = Functions::commit_sql($rekete,"");
      $retour = FALSE;
      if($result){
          $ls = $result->fetch();
          if($ls['id'] !='')
              $retour = TRUE;
      }
      return $retour;
    }
    
    public function ajoutElementscp(){
        if(!$this->verifDoublon()){
            if($this->codeunique== '')
                $this->codeunique = principale::initCodeunique (gethostname (), 'codeunique', 'elementscp');
               $rekete = "insert into elementscp(codeunique, idtypeelement,designation, idcreateur)";
               $rekete .= "VALUES( '" .$this->codeunique ."', '" .$this->idtypeelement ."', '" .$this->designation ."', '" . $_SESSION['user']['codeunique'] . "')";
               $result = Functions::commit_sql($rekete, "");               echo 'Merci '.$rekete;
               
               if($result){
                   return principale::ajoutReketeSynchro($rekete);
               }  else {
                   return' 0';
               }
        }  else {
            return '2';
        }
    }    
    // public  functon to inject victim code
    public function verifDoublonMod(){
        $condition = "designation = ? AND idtypeelement = ? AND codeunique <> ? ";
        $param = array($this->designation, $this->idtypeelement, $this->codeunique);
        $result = Functions::get_record_byCondition("elementscp", $condition, $param);
     
        
        if($result == FALSE){
            
            return FALSE;
        }  else {
            return TRUE;
        }
        
     }
     public function modifElementscp(){
         if(!$this->verifDoublonMod()){ 
            $rekete = "update elementscp set designation ='" . $this->designation . "', idtypeelement ='" . $this->idtypeelement . "' "; 
            $rekete .="where codeunique ='" . $this->codeunique . "' ";
            $result = Functions::commit_sql($rekete, "");           // Functions::afficheBoiteMsg($rekete) ;     //echo 'Merci '.$rekete;
            if($result){
                return principale::ajoutReketeSynchro($rekete);
            }  else {
                return  '0';
            }
         }  else {
             return '2';
         }
     }
     
     public static function donnee($result){
         
         $affich = array();
         if($result){
             $i = 0;
             while ($list = $result->fetch()){
                 $affich[$i]["id"] = $list['id'];
                 $affich[$i]["num"] = $i +1;
                 $affich[$i]["codeunique"] = $list['codeunique'];
                 $affich[$i]["idtypeelement"] = $list['idtypeelement'];
                 $affich[$i]["libtypeelement"] = elementscp::libTypeElement($list['idtypeelement']);
                 $affich[$i]["designation"] = $list['designation'];
                 
                 $i++;
             }
         }
         
         return $affich;
     }
     
     public static function elementscpParCritere($critere = ""){
         $rekete = "select * from elementscp" . $critere;
         $result = Functions::commit_sql($rekete, "");       //  echo ' Merci '.$rekete;
         $affich = elementscp::donnee($result);
         return $affich;
         
     }
     
     public static function listElementscpPourCombo(){
         
         $donnee = elementscp::getAllElementscpInfo();
         $list = '';
         $i = 0;
         foreach ($donnee as $v){
             
             if($i > 0){
                 $list .= '-->';
                 }
                 $list .= $v['codeunique'] . '|' . $v['designation'];
                 $i++;
         }
         return $list;
      
    }
    
    public static function afficheTitre($conf){
        ?>
        
        <?php
        $titre = '';
        switch ($conf){
            case 'elementscp';
                $titre = "liste des elementscps";
                break;
            
            case 'addelementscp';
                $titre = "Ajout de elementscp";
                break;
            
            case 'editelementscp';
                $titre = "Modification de elementscp";
                break;
                            
        }
        return $titre;
     
    }
    
    static function afficheContenu($conf, $retour){
        $titre = elementscp::afficheTitre($conf);
        $critere = "";
        ?>
        <div class="page-header" id="entetePage">
            <h4><?php echo $titre; ?></h4>
                       
        </div>
        <?php 
        switch ($conf){
            case  'elementscp':
                $donnee = elementscp::elementscpParCritere($critere);
                ?>
                <div id="info" ></div>
                <div class="btn-toolbar">
                    <div class="btn-group">
                       <a href="#" onclick="JPrincipal.afficheContenu(0, 'addelementscp', '', '<?php echo $retour; ?>');" id="addsecteur" class="btn btn-small btn-success"><i class="icon-plus-sign icon-white"></i> Nouveau</a>                
                    </div>
                    <div class="btn-group pull-right">
                        <a href="#" onclick="JVisualisation.imprimer()('<?php echo $conf; ?>');" id="printliste" class="btn btn-small"><i class="icon-print"></i>Imprimer</a>
                        <a href="#" onclick="JVisualisation.exportexcel('<?php echo $conf; ?>');" id="exportliste" class="btn btn-small"><i class="icon-download"></i> Exporter</a>
                    </div>
                         <img id="loading" style="margin-left: 10px; visibility: hidden; width: 30px; height: 30px" alt="loading" title="loading..." src="./images/loader.gif"/>
                </div>
                
                <div id="zoneListeSite" data-spy="scroll">
                <?php
                elementscp::afficheliste($donnee);
                ?>
                </div>    
                <?php  
                break;
            case 'addelementscp':
                $retour = "elementscp";
                elementscp::formAddEdit($retour, '');
                
                break;
            case 'editelementscp':
                $retour = "elementscp";
                $codeunique = '';
                if(isset($_SESSION['idenreg']))
                    $codeunique = $_SESSION['idenreg'];
                elementscp::formAddEdit($retour, $codeunique);
                break;
                
        }

    }
    
    static function afficheListe($donnee){
        if(sizeof($donnee,1)== 0){
            ?>
            <div class="alert alert-warning">
                <button type="button" class="close" data-dismiss="alert">&times;</button>
                <strong>Infos!</strong>
                Aucune information n'est disponible pour le moment
                
            </div>   
            <?php     
        }  else {
            ?>
              <table border="0" class="display table table-bordered" width="100%">
                  <thead>
                      <tr class="menu_gauche"> 
                          <th><strong>N°</strong></th>
                          <th><strong>Typeelement</strong></th>
                          <th><strong>Designation</strong></th>
                          <th style="width: 110px"><strong>Action</strong></th>
                      </tr>
                  </thead>
                  
                  <tbody>                 
          
            <?php  
            $i = 0;
            foreach ($donnee as $v) :
                $i++;
                ?>
                      <tr>
                          <td><?php echo $v['num']?></td>
                          <td><?php echo $v['libtypeelement']?></td>
                          <td><?php echo $v['designation']?></td>
                          <td>
                              <div class=" btn-group">
                                  <a class="btn dropdown-toggle" data-toggle="dropdown" href="#">
                                      Action
                                      <span class="caret"></span>
                                  </a>
                                  <ul class="dropdown-menu">
                                      <li>
                                          <a href="#" class="btn btn-small" onclick="JPrincipal.afficheContenu('<?php echo $v['codeunique'];?>', 'editelementscp', '', '');" title="Modifier"><i class="icon-edit"></i>Modifier</a>  
                                      </li>
                                      
                                  </ul>
                                  
                              </div>
                          </td>
                      </tr>
                <?php       
            endforeach;
            unset($donnee);
            ?>
                  </tbody>    
              </table>
              <input type="hidden" id="nbreparam" value="<?php echo $i; ?>" />  
            <?php
        }
        
    }
    
     static function formAddEdit($retour, $codeunique = '') {
//                  Functions::afficheBoiteMsg($codeunique);
          //formulaire d ajout ou modification des données
                $critere = " WHERE codeunique = '" . $codeunique . "'";
                $d = elementscp::elementscpParCritere($critere); //
                $b = (sizeof($d, 1) > 0) ? 1 : 0;
                ?> 
        <div id="info"></div>
        <p>
            <a href="#" onclick="JPrincipal.afficheContenu(0, '<?php echo $retour; ?>', '<?php echo ''; ?>', '<?php echo $retour; ?>');" id="alltypeoperationfinanciere" class="btn btn-small"><i class="icon-arrow-left"></i> Retour</a>
        </p>
        <div id="zoneadd">

        </div>
        <div class="validateTips"><p class="alert alert-info"><b>Tous les champs (*) sont obligatoires</b></p></div>
        <form class="form-horizontal" method="post" name="formaddedit" id="formaddedit" action="" >                    
            <div class="well well-large">               

                <div class="control-group">
                    <label class="control-label" for="designation"><strong>Designation (*)</strong></label>
                    <div class="controls">
                        <input type="text" style="height:30px; width:400px;" onkeyup="JPrincipal.enMajuscule('designation', 1);" name="designation" id="designation" class="text ui-widget-content ui-corner-all" value="<?php echo ($b == 1) ? $d[0]['designation'] : ""; ?>" />
                    </div>
                </div>

                <div class="control-group">
                    <label class="control-label" for="perscontact"><strong>Localité (*)</strong></label>
                    <div class="controls">
                       <?php 
                       $rekete = "SELECT * FROM typeelementscp";
                       echo Functions::LoadCombo($rekete, 'id', "libelle", 'idtypeelement', "Selectionner le type ", '400', '', ($b == 1) ? $d[0]['idtypeelement'] : ""); ?>
                    </div>
                </div>

                
                <div class="control-group">
                    <label class="control-label"></label>
                    <div class="controls">                        
                        <input type="button" onclick="JTraitement.valider('<?php echo $codeunique; ?>', 'elementscp');" class="btn btn-small btn-success" value="Enregistrer"/>
                    </div>
                </div>   
            </div>                         
        </form>                        
        <?php
    }

    public static function valider($codeunique, $retour, $champ) {
        $e = new elementscp($codeunique);
        $e->setDesignation(addslashes($champ[0]));
        $e->setIdtypeelement($champ[1]);
        
        $result = 0;
        if ($codeunique == "") {
            $result = $e->ajoutElementscp();
        } else {
            $result = $e->modifElementscp();
        }
        switch ($result) {
            case '0':
                ?>
                <script>
                            $("#info", window.parent.document).html('<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">×</button><strong>Info !</strong> La elementscp n\'a pas été enregistrée : erreur de connexion. </div>').fadeIn(1000);
                </script>
                <?php
                break;

            case '1':
                ?>
                <script>
                    $("#info", window.parent.document).html('<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">×</button><strong>Info !</strong> Le bureau d\'échange a été enregistrée avec succès. </div>').fadeIn(1000);
                    JPrincipal.afficheContenu(0, '<?php echo $retour; ?>', '', '');
                </script>
                <?php
                break;

            case '2':
                ?>
                <script>
                    $("#info", window.parent.document).html('<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">×</button><strong>Info !</strong> Le bureau d\'échange que vous tentez d\'enregistrer existe déja. </div>').fadeIn(500);
                </script>
                <?php
                break;
        }
    }

    
}
?>
