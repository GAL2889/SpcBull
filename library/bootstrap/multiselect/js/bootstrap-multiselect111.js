...
!function ($) {
	...
	function Multiselect(select, options) {
		...
		this.$zonemodal = $(this.options.buttonZonemodal),
		...
		if (this.options.buttonWidth) {
			$('button', this.$container, this.$zonemodal).css({
		...
		if (this.options.maxHeight) {
		    $('.multiselect-container ul', this.$container, this.$zonemodal).css({
				'max-height': this.options.maxHeight + 'px',
				'overflow-y': 'auto',
				'overflow-x': 'hidden'
			});

			$('input[type="text"]', this.$container, this.$zonemodal).width('75%');
		}
		
		// Enable filtering.
		if (this.options.enableFiltering) {
		    $('.multiselect-container', this.$container, this.$zonemodal).prepend('<div class="input-prepend" style="padding:3px;"><span class="add-on"><i class="icon-search"></i></span><input class="multiselect-search" type="text" placeholder="' + this.options.filterPlaceholder + '"></div>');
		    
		    $('.multiselect-search', this.$container, this.$zonemodal).val(this.query).on('click', function (event) {
		        ...
		        this.searchTimeout = this.asyncFunction($.proxy(function () {
		            ...
		            if (this.query != event.target.value) {
		                ...
		                $.each($('ul li', this.$container, this.$zonemodal), $.proxy(function(index, element) {
